...
// tokenize entered input code from https://brennan.io/2015/01/16/write-a-shell-in-c/
...
// lanuch function  https://stackoverflow.com/questions/20432595/creating-a-background-process-for-shell-in-c & http://www.microhowto.info/howto/capture_the_output_of_a_child_process_in_c.html
...
pipe(filedes);
 ...
 for (i=0; args[i]!=NULL; i++){
	...
	if(strchr(cmd_line, '&') != NULL) { // command is background &
    background = 1;
	args[i-1] = NULL; // decrease argc by one,set the argument that had the bg to null
...
// now we have cmd in t
// Put in shell to parse command, fork(), execvp(), then optional wait() this is where we put the shell code 

   args = tokenize_input(t,SHELL_LINE_DELIM); //parse t into args 
   out.status = shell_execute(args); // then execute 
   free(t);
   free(args);
